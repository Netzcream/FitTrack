...
@php
$base = 'inline-flex items-center gap-2 font-medium focus:ring-4 focus:outline-none transition rounded-lg';
...
$variants = [
    ...
    'alternative' => 'text-gray-900 bg-white border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:ring-gray-100 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-700',
...
$class = "{$base} {$sizes[$size]} " . ($variants[$variant] ?? $variants['primary']);
@endphp
...
@else
    <button {{ $attributes->merge(['type' => $type, 'class' => $class]) }} @if($disabled) disabled @endif>
        @if($icon)
            <x-icon name="{{ $icon }}" class="w-4 h-4" />
        @endif
        {{ $slot }}
