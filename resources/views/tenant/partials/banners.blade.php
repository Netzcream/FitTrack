@if (tenant_config('landing_banners_show'))
    ...
    @php
        ...
        $bannerData = $banners->map(function ($banner) {
            return [
                'uuid' => $banner->uuid,
                'image' => $banner->getFirstMediaUrl('cover') ?: asset('images/default-banner.jpg'),
                'link' => $banner->link,
    ...
    @if (count($banners) > 0)
        <div x-data="{
        ...
        }" class="w-full h-[250px] md:h-[350px] lg:h-[500px] relative overflow-hidden">

            <template x-for="(banner, index) in banners" :key="banner.uuid">
                <div class="absolute inset-0 transition-opacity duration-700" x-show="index === activeIndex"
                    ...
                    <a :href="banner.link" :target="banner.target" class="relative block w-full h-full group">
                        ...
                        <template x-if="banner.text">
                            <div
                                ...
                                <p class="text-white text-center text-xl md:text-6xl font-semibold leading-snug"
                                    x-text="banner.text"></p>
                            </div>
                        </template>
                    </a>
            ...
            <div class="absolute inset-y-0 left-0 flex items-center px-4">
                <button @click="activeIndex = (activeIndex - 1 + banners.length) % banners.length; resetInterval()"
            ...
            <div class="absolute inset-y-0 right-0 flex items-center px-4">
                <button @click="activeIndex = (activeIndex + 1) % banners.length; resetInterval()"
                    class="bg-black/40 text-white p-2 rounded-full hover:bg-black/60 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor" stroke-width="2">
