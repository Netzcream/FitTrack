  ...
  <div class="flex h-full w-full flex-1 flex-col gap-6">
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4 flex flex-col">
        ...
        <p class="text-xs text-neutral-500 mt-auto pt-3">{{ __('site.updated') }}: <span class="font-medium text-neutral-700 dark:text-neutral-300">hoy</span></p>
      </div>
      ...
      <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4 flex flex-col">
        ...
        <div class="rounded-lg bg-neutral-100 dark:bg-neutral-800 p-2"><flux:icon.activity class="size-5" /></div>
        </div>
        <p class="text-xs text-neutral-500 mt-auto pt-3">{{ __('site.updated') }}: <span class="font-medium text-neutral-700 dark:text-neutral-300">hoy</span></p>
      </div>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-3">
      ...
      <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4 lg:col-span-2">
        <div class="flex items-center justify-between">
          <h3 class="text-sm font-semibold">{{ __('site.published_per_week') }}</h3>
          <a wire:navigate href="{{ route('tenant.dashboard.students.index') }}" class="text-xs text-[var(--brand-700)] hover:underline">{{ __('site.view_all') }}</a>
        </div>

        @php $total = array_sum(array_column($publishedLast8Weeks, 'value')); @endphp

        @if ($total === 0)
          <div class="h-40 flex items-center justify-center text-sm text-neutral-500">{{ __('site.no_data_yet') }}</div>
        @else
          <div class="mt-4 h-40 flex items-end gap-3">
            @foreach ($publishedLast8Weeks as $week)
              @php
                $h = max(8, (int) round(($week['value'] / max(1, $publishedPeak)) * 100));
              @endphp
              <div class="flex-1 flex flex-col items-center">
                <div class="w-full max-w-6 mx-auto bg-[var(--brand-700)]/85 dark:bg-[var(--brand-600)]/90 rounded-t"
                  style="height: {{ $h }}%"></div>
                <div class="mt-1 text-[10px] text-neutral-500">{{ $week['label'] }}</div>
              </div>
            @endforeach
          </div>
        @endif
      ...
      <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <flux:modal.trigger name="create-student">
            <flux:button class="!h-auto w-full flex flex-col items-center justify-center py-6 text-center" type="button">
          ...
          <flux:button as="a" href="#" class="!h-auto flex flex-col items-center justify-center py-6 text-center col-span-2">
      ...
      <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 p-4 lg:col-span-2">
        <div class="flex items-center justify-between">
          ...
          <div class="flex items-center gap-2">
            ...
            <a wire:navigate href="{# route('tenant.dashboard.messages.index') #}" class="text-xs text-[var(--brand-700)] hover:underline">{{ __('site.view_all') }}</a>
          </div>
        </div>
        ...
        <div class="mt-3 overflow-x-auto">
          <table class="min-w-full text-sm">
            ...
            <tr>
              <td colspan="4" class="py-6 text-center text-neutral-500">{{ __('site.no_contacts_yet') }}</td>
            </tr>
            </tbody>
          </table>
        </div>
