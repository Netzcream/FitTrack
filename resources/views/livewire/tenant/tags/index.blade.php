  ...
  <div class="flex-1 self-stretch max-md:pt-6">
    <div class="relative mb-6 w-full">
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <flux:heading size="xl" level="1">{{ __('site.tags') }}</flux:heading>
          <flux:subheading size="lg" class="mb-6">
            {{ __('site.tags_subheading') }}
          </flux:subheading>
        </div>
        <flux:button as="a" href="{{ route('tenant.dashboard.tags.create') }}"
        ...
        @forelse ($tags as $tag)
          <tr>
            ...
            <td class="align-top px-6 py-4 text-sm">
              <span class="text-xs {{ $tag->is_active ? 'text-green-600 dark:text-green-400' : 'text-gray-500' }}">
                {{ $tag->is_active ? __('site.yes') : __('site.no') }}
              </span>
            </td>
            <td class="align-top px-6 py-4 text-end text-sm font-medium">
              <span class="text-xs text-gray-400 dark:text-neutral-500 inline-flex items-center whitespace-nowrap">
                <flux:button wire:navigate size="sm"
                  href="{{ route('tenant.dashboard.tags.edit', $tag) }}">
                  {{ __('site.edit') }}
                </flux:button>
                <flux:modal.trigger name="confirm-delete-tag">
        ...
        @empty
          <tr>
            <td colspan="4" class="px-6 py-4 text-sm text-center text-gray-500 dark:text-neutral-400">
              {{ __('site.no_tags_found') }}
            </td>
          </tr>
        @endforelse

        <x-slot name="modal">
          <flux:modal name="confirm-delete-tag" class="min-w-[22rem]" x-data
            @tag-deleted.window="$dispatch('modal-close', { name: 'confirm-delete-tag' })">
            <div class="space-y-6">
              <div>
                <flux:heading size="lg">{{ __('site.delete_tag_title') }}</flux:heading>
                <flux:text class="mt-2">
                  {{ __('site.delete_tag_message') }}
                </flux:text>
              </div>
