<div class="container mx-auto px-4 py-6">
    {{-- Header --}}
    <div class="mb-6">
        <flux:heading size="xl" level="1">{{ __('Mensajes con tu entrenador') }}</flux:heading>
        <flux:subheading size="lg">{{ __('Mantente en contacto con tu equipo de entrenamiento') }}</flux:subheading>
    </div>
            ...
            <div wire:poll.3s class="space-y-4" id="messages-container-student">
                @forelse ($messages as $message)
                    <div class="flex {{ $message->sender_type->value === 'student' ? 'justify-end' : 'justify-start' }}">
                        <div class="max-w-[70%] {{ $message->sender_type->value === 'student' ? 'bg-blue-500 text-white' : 'bg-white dark:bg-neutral-800 text-gray-900 dark:text-neutral-100' }} rounded-lg px-4 py-2 shadow-sm border {{ $message->sender_type->value === 'student' ? '' : 'border-gray-200 dark:border-neutral-700' }}">
                            <div class="text-sm break-words whitespace-pre-wrap">{{ $message->body }}</div>
                            <div class="text-xs {{ $message->sender_type->value === 'student' ? 'text-blue-100' : 'text-gray-500 dark:text-neutral-400' }} mt-1">
