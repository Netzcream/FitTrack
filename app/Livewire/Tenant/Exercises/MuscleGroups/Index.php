...
class Index extends Component
{
    ...
    public ?int $muscleGroupToDelete = null;
    ...
    {
        $this->muscleGroupToDelete = $id;
    }

    public function delete(): void
    {
        if (!$this->muscleGroupToDelete) return;

        if ($group = MuscleGroup::find($this->muscleGroupToDelete)) {
            $group->delete();
        }

        $this->dispatch('muscle-group-deleted');
        $this->reset('muscleGroupToDelete');
    }
    ...
    {
        /** @var MuscleGroup $current */
        $current = MuscleGroup::findOrFail($id);

        if ($current->order === null) {
            $max = (int) MuscleGroup::max('order');
            $current->order = $max + 1;
            $current->save();
        }

        $minOrder = (int) (MuscleGroup::min('order') ?? 0);
        $maxOrder = (int) (MuscleGroup::max('order') ?? 0);

        if ($direction === 'up') {
            if ($current->order <= $minOrder) return;
            $neighbor = MuscleGroup::where('order', '<', $current->order)
    ...
    public function render()
    {
        ...
        $muscleGroups = (clone $base)
            ->orderByRaw('CASE WHEN `order` IS NULL THEN 1 ELSE 0 END, `order` ASC')
            ->orderBy('name')
            ->paginate($this->perPage);

        return view('livewire.tenant.exercises.muscle-groups.index', [
